		...
		List<string> convertImgToPdf()
		{
			...
			if(mSeparate)
			{
				...
				while(i < AllPages)
				{
					...
					convertImgToPdf(path, mImagePaths[i]);
					
					PDF_Files.Add(path);

					// This is incremented by "OCRManager_ProgressUpdated()"
					//CurrentPage++;
			...
			}else
			{
				...
				path = convertImgToPdf(path, mImagePaths);
...
//---------------------------------------------------------------------------------
		...
		string convertImgToPdf(string dst, List<string> src)
		{
			...
			if(mOcr)
			{
				...
				OCRManager.GetPDF(src, dst, Path.GetFileName(dst));
