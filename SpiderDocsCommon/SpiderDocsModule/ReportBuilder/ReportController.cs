...
//            conn = SpiderDocsModule.SqlOperation.DBConnectionString;
        ...
        public int AddEmptyReport(int userId)
        {
            ...
            try
            {
                ...
                {
                    sqlConn.Open();
                    string sql = "INSERT INTO reporting.Reports (Report_Name, User_Id, Created_Date, Active,Category_Id) ";
                    sql += " OUTPUT INSERTED.ID VALUES (NULL, @userId, GETDATE(), 0,0) ";
        ...
        public bool UpdateReport(int id, string reportName, bool active, int categoryId = 0)
        {
            bool ret = false;
            ...
            try
            {
                ...
                {
                    sqlConn.Open();
                    string sql = "UPDATE reporting.Reports SET Report_Name = @reportName, Active = @active, Category_Id = @categoryId WHERE Id = @id ";
		...
		public List<Reporting_Fields> GetFieldsBy()
		{
			List<Reporting_Fields> list = new List<Reporting_Fields>();
			...
			try
			{
				...
				{
					sqlConn.Open();
					string sql = "SELECT * FROM reporting.Fields WHERE Active = 1";
        ...
        public List<Reporting_Comparator> GetFieldComparators()
        {
           List<Reporting_Comparator> list = new List<Reporting_Comparator>();
           ...
           try
           {
               ...
               {
                   sqlConn.Open();
                   string sql = "SELECT c.* FROM reporting.view_rpt_field_comparator c ";
        ...
        public string CreateDynamicReport(int reportId, string strPath = null)
        {

            // TODO: change username
            strPath = strPath + "\\" + GetReportById(reportId).Report_Name + "_" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".csv"; //strPath ?? GetReportFileName(GetReportById(reportId).Report_Name, SpiderDocsModule.SpiderDocsApplication.CurrentUserName);
            try
            {
        ...
        public string GetReportFileName(string reportName, string LoggedBy,string path)
        {

            string strPath = "";
            if (Directory.Exists(SpiderDocsModule.FileFolder.GetTempFolder() ))
            {
                // Step 0: Create Greentree folder in temp
                string root = Path.Combine(SpiderDocsModule.FileFolder.GetTempFolder(), "Reports");
                if (Directory.Exists(root) == false)
                {
                    Directory.CreateDirectory(root);
                }
                    ...
                    using (SqlCommand command = new SqlCommand(sql, sqlConn))
                    {
                        ...
                        using (SqlDataReader dr = command.ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                ...
                                rr.Category_Id = (dr["Category_Id"] == DBNull.Value) ? 0 : int.Parse(dr["Category_Id"].ToString());
                            }
                        }
                    }
