...
namespace PresentationControls
{
    ...
    public partial class CheckBoxComboBox : PopupComboBox
    {
		...
		bool OnCheckBoxCheckedChangedRunning = false;
        protected void OnCheckBoxCheckedChanged(object sender, EventArgs e)
        {
			if(OnCheckBoxCheckedChangedRunning)
				return;

			OnCheckBoxCheckedChangedRunning = true;
			...
			if(!MultiSelectable)
			{
				int cnt = CheckBoxItems.Count;
				for(int i = 0; i < cnt; i++)
				{
					if(CheckBoxItems[i] != (CheckBoxComboBoxItem)sender)
			...
			OnCheckBoxCheckedChangedRunning = false;
        ...
        {
            Size Size = new Size(dropDown.Width, DropDownControl.Height);
            dropDown.Size = Size;
            base.OnResize(e);
        }
		...
		public List<T> getComboValue<T>()
		{
			...
			{
				ListSelectionWrapper<T> data = (ListSelectionWrapper<T>)this.DataSource;

				int cnt = data.Count;
				for(int i = 0; i < cnt; i++)
				{
					if(data[i].Selected)
						vals.Add(data[i].Item);
				}
			}
